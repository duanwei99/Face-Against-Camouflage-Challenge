...
class PyFAT:
    ...
    def __init__(self, N=10):
        ...
        self.num_images = 2 # 每一次迭代选择的图片数量
        self.alpha = (1.0 / 255)
    ...
    def load(self, assets_path):
        ...
        # load face mask
        mask_np = cv2.resize(cv2.imread(osp.join(assets_path, 'mask_0524.png')), img_shape) / 255
    ...
    def generate(self, im_a, vic_path, n):
        ...
        for i in range(self.epoch):
            # Shuffle
            indices = np.random.randint(0, self.num_images, (self.num_images,))
            sf_vic_feats_ir50 = []
            sf_vic_feats_ir100 = []
            for j in range(self.num_images):
                sf_vic_feats_ir50.append(vic_feats_ir50_sub[indices[j]])
                sf_vic_feats_ir100.append(vic_feats_ir100_sub[indices[j]])
            sf_vic_feats_ir50 = torch.stack(sf_vic_feats_ir50)
            sf_vic_feats_ir100 = torch.stack(sf_vic_feats_ir100)
            ...
            for j in tqdm(range(self.num_images)):
                # Attack IResNet50
                adv_image = self.attack_model(adv_image, sf_vic_feats_ir50[j], 'ir50', loss_func)

                # Attack IResNet100
                adv_image = self.attack_model(adv_image, sf_vic_feats_ir100[j], 'ir100', loss_func)

        # Calculate cosine similarity
        adv_feats_ir50 = self.model1.forward(adv_image)
        adv_feats_ir100 = self.model2.forward(adv_image)
        sims_ir50 = []
    ...
    def attack_model(self, adv_image, vic_feats, model_name, loss_func):
