...
class PyFAT:
    ...
    def load(self, assets_path):
        ...
        # load face mask    
        mask_np = cv2.resize(cv2.imread(osp.join(assets_path, 'mask_2.png')), img_shape) / 255
    ...
    def generate(self, im_a, im_v, n):
        ...
        # cv2.circle(img_test,(int(bboxes[0][0]),int(bboxes[0][1])),2,(0,0,255),-1)
        # cv2.circle(img_test, (int(bboxes[0][2]), int(bboxes[0][3])), 2, (0, 0, 255), -1)
        s = (bboxes[0][2] -bboxes[0][0]) * (bboxes[0][3] -bboxes[0][1])
        ...
        for i in tqdm(range(self.num_iter)):
            ...
            loss = torch.sum(torch.abs(adv_feats-vic_feats).pow(3))
        ...
        # att_img = att_img.cpu().detach().squeeze().numpy().transpose(1,2,0)
        # att_img = 0.5 * att_img +0.5
        # att_img = att_img *255
        ...
        return adv_img
        # diff = att_img - att_img_
        # diff = diff.cpu().detach().squeeze().numpy().transpose(1, 2, 0) * 0.5 + 0.5
        # diff *= 255
